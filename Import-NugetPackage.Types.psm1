...
class ImportNugetPackageOutputBase {
    ...
    [object] CreateInstanceOfType(
        ...
        [Func[string, string, object[], object]] $instanceCreator
    ) {
        foreach ($assembly in $assemblies) {
            ...
            if ($null -ne $type) {
                ...
                if ($null -eq $paramss) {
                    return $instanceCreator.Invoke($typeName, $assembly.FullName, $null)
                }
                
                return $instanceCreator.Invoke($typeName, $assembly.FullName, $paramss)
            }
        }
...
if ($isDotnetCore) {
    class ScriptAssemblyContext : System.Runtime.Loader.AssemblyLoadContext {
        ...
        [object] CreateObjectOfType([string] $typeName, [object[]] $paramss = $null) {
            ...
            $assemblyList = [IEnumerable[Assembly]] $([AssemblyLoadContext]::Default.Assemblies)
            $obj = $this.CreateInstanceOfType($assemblyList, $typeName, $paramss);
